...
# Simulate system failure with a 20% chance
def simulate_system_failure():
    return random.random() < 0.2  # 20% chance of failure

# Simulate the visitor information process with error handling
def simulate_visitorInfo_process(num_cases):
    ...
    for _ in range(num_cases):
        ...
        # Step 2: VisitorInformationSystem responds based on the user's activity, with possible failure
        if activity == "Click specific hot attraction read more button":
            if simulate_system_failure():
                activity = "Error: Unable to retrieve specific attraction information"
            else:
                activity = service_provider_activities[1]  # Respond with specific attraction information
        elif activity == "Browse attraction page":
            if simulate_system_failure():
                activity = "Error: Unable to retrieve all attraction information"
            else:
                activity = service_provider_activities[3]  # Respond with all attraction information
        else:  # Search for attraction
            if simulate_system_failure():
                activity = "Error: Unable to retrieve searched attraction information"
            else:
