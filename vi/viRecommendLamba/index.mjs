...
const resultTableName = "PrecomputedAttractions"; // Results table

export const handler = async (event) => {
  console.log("Received event:", JSON.stringify(event, null, 2));

  // DynamoDB query parameters to fetch all attractions from the results table
  const scanParams = {
    TableName: resultTableName,
  };

  try {
    // Execute the Scan command to retrieve all items from the results table
    ...
    // Return the fetched attractions
    return {
      statusCode: 200,
      headers: {
