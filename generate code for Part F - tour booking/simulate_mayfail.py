...
# Simulate the booking process with errors
def simulate_booking_process(num_cases):
    all_entries = []
    case_counter = 1  # Start case counter from 1
    ...
    for _ in range(num_cases):
        ...
        # Step 1: User chooses to browse or search, simulate user exit error
        if random.random() < 0.1:  # 10% chance user exits after browsing/searching
            ...
            all_entries.append([case_id, end_time, end_time, "Process terminated: User exited early", user])
            continue  # Case ends here
        ...
        # Step 2: VisitorInformationSystem responds, simulate system failure
        if random.random() < 0.05:  # 5% chance system failure
            all_entries.append([case_id, current_time, current_time, "Process terminated: System failure", "VisitorInformationSystem"])
            continue  # Case ends here
        ...
        activities_order = [
            ...
            (user_activities[5], user),  # Payment step, simulate payment failure
        ...
        for i, (activity, resource) in enumerate(activities_order):
            if i == 8 and random.random() < 0.1:  # 10% chance of payment failure at step 8
                all_entries.append([case_id, current_time, current_time, "Process terminated: Payment failure", "PaymentGateway"])
...
def write_to_csv(entries):
    ...
    df.to_csv('tour_booking_log.csv', index=False)
    print(f"Simulation complete. Check 'tour_booking_log.csv' for the results.")

# Main execution
if __name__ == "__main__":
    num_cases = 100  # Number of booking processes to simulate
